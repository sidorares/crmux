      ...
      upRes.pipe(
        bl(function(err, data) {
          ...
          for (var i = 0; i < tabs.length; ++i) {
            ...
            if (typeof wsUrl == 'undefined') continue;
            ...
            if (typeof feUrl == 'undefined') continue;

            tabs[i].webSocketDebuggerUrl = replaceHost(wsUrl, res);
            if (tabs[i].devtoolsFrontendUrl)
              tabs[i].devtoolsFrontendUrl = tabs[i].devtoolsFrontendUrl.replace(
                wsUrl.slice(5),
                tabs[i].webSocketDebuggerUrl.slice(5)
              );
...
var server = http.createServer(function(req, res) {
  if (req.url == '/json') {
    ...
    http
      .request(options, function(upRes) {
...
wss.on('connection', function(ws, request) {
    ...
    upstreamSocket.on('message', function(message) {
      ...
      if (!msgObj.id) {
        ...
        upstreamMap[wsUpstreamUrl].clients.forEach(function(s) {
          if (program.debug) console.log('e> ' + message.cyan);
  ...
  ws.on('message', function(message) {
    ...
    } else upstream.send(JSON.stringify(msgObj));
  });
  ...
  var removeFromUpstreamMap = function() {
    ...
    var purged = upstreamMap[wsUpstreamUrl].clients.filter(function(s) {
      return s._id != ws._id;
    });
